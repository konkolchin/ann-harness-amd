...
\subsection{FAISS vs VectorDBBench Milvus on same host (7900 XTX)}

Tables~5 and~6 run on \texttt{amd-rx7900xtx} with the same SIFT-1M L2 dataset and IVF\_FLAT
\texttt{nlist/nprobe} ladder; this is our cleanest FAISS-vs-Milvus CPU comparison to date:
\begin{table}[h]
\centering
\caption{FAISS vs VectorDBBench Milvus (7900 XTX, comparable recall@10)}
\begin{tabular}{@{}lrrrr@{}}
\toprule
\textbf{Target recall@10} & \textbf{FAISS QPS} & \textbf{FAISS nprobe}
  & \textbf{Milvus QPS} & \textbf{Milvus nprobe} \\
\midrule
$\approx 0.37$ & 89{,}993 & 1  & 1{,}589 & 1 \\
$\approx 0.70$ & 24{,}485 & 4  & 1{,}219 & 4 \\
$\approx 0.83$--$0.84$ & 12{,}511 & 8  & 937 & 8 \\
$\approx 0.93$ & 6{,}409 & 16 & 630 & 16 \\
$\approx 0.98$ & 3{,}299 & 32 & 376 & 32 \\
\bottomrule
\end{tabular}
\end{table}

FAISS remains ahead on QPS at every matched recall point, but the gap on this host is
smaller than Table~1 vs Table~4 (cross-host). VectorDBBench adds optimize/compact overhead
on the first load; p99 latencies in Table~6 are sub-millisecond to a few ms (serial protocol).
...
\begin{itemize}[leftmargin=*]
  ...
  \item For an \texttt{nprobe} sweep comparable to FAISS via VectorDBBench, use
        Section~\ref{sec:vectordbbench-ivf-nprobe} (\texttt{milvusivfflat}, not AutoIndex).
\end{itemize}
