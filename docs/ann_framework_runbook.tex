...
\texttt{nlist=1024}, and \texttt{nprobe} sweep ($1,4,8,16,32$). Because recall@10 tracks
FAISS within rounding at each \texttt{nprobe}, we can compare QPS and recall side by side
row-by-row (Milvus via VectorDBBench \texttt{milvusivfflat} on Standalone Docker CPU):
\begin{table}[h]
\centering
\caption{FAISS vs Milvus Standalone (7900 XTX): IVF \texttt{nprobe} sweep (\texttt{k=10})}
\begin{tabular}{@{}lrrrr@{}}
\toprule
\textbf{nprobe} & \textbf{FAISS QPS} & \textbf{FAISS recall@10}
  & \textbf{Milvus QPS} & \textbf{Milvus recall@10} \\
\midrule
1  & 89{,}993.4 & 0.3711 & 1{,}589.0 & 0.3704 \\
4  & 24{,}484.8 & 0.6983 & 1{,}219.4 & 0.7017 \\
8  & 12{,}510.7 & 0.8337 &   937.4 & 0.8390 \\
16 &  6{,}408.9 & 0.9273 &   630.1 & 0.9312 \\
32 &  3{,}298.5 & 0.9760 &   375.9 & 0.9790 \\
\bottomrule
\end{tabular}
\end{table}

FAISS FlatL2 (exact search) on this host: QPS $=$ 2{,}340.8, recall@10 $=$ 1.0000 (no Milvus
row---VectorDBBench run used IVF\_FLAT only). FAISS remains ahead on QPS at every
\texttt{nprobe}, but the gap on this host is smaller than Table~1 vs Table~4 (cross-host).
VectorDBBench adds optimize/compact overhead on the first load; p99 latencies in Table~6 are
sub-millisecond to a few ms (serial protocol).
...
These measured CPU tables intentionally omit Milvus strengths that FAISS does not provide out of
the box:
\begin{itemize}[leftmargin=*]
...
\begin{itemize}[leftmargin=*]
  \item Tables~1--6 do \textbf{not} show Milvus outperforming FAISS on search QPS; for
        single-node CPU IVF, FAISS is ahead (same-host proof: Table~5 vs Table~6 on 7900 XTX).
  ...
  \item Milvus Standalone confirms proper \texttt{nprobe} behavior but is slower and
        heavier than in-process FAISS; on 7900 XTX, VectorDBBench Milvus reaches
        $\approx$376 QPS at recall@10 $\approx 0.98$ vs $\approx$138 on WSL2 harness (Table~4).
  \item The harness goal is a \textbf{reproducible baseline before AMD GPU work}, not to
        declare a winner on raw QPS.
  \item Table~5 vs Table~6 (7900 XTX, 2026-06-30) satisfies the same-host CPU comparison;
        next step is GPU Milvus (hipVS) on the same machine once ROCm is installed.
\end{itemize}
...
\begin{itemize}[leftmargin=*]
  ...
  \item Install ROCm on the 7900 XTX host and verify GPU compute
        (\texttt{/opt/rocm/bin/rocminfo}); then validate hipRAFT/hipVS and Milvus GPU path on
        ROCm~7.x.
  \item Optionally run \texttt{run\_milvus\_hdf5.py} on the same host for harness metrics
        comparable to WSL2 Table~4 (Insert/Search/\texttt{search\_with\_expr}).
  \item Extend benchmark matrix to GIST/Deep subsets (harness and/or VectorDBBench).
  \item After hipVS lands in Knowhere/Milvus GPU builds, repeat the Table~5/6 methodology on AMD
        GPU and compare against the CPU baselines already captured on this host.
\end{itemize}
