...
\paragraph{Why use Milvus if FAISS is faster?}
This table measures \textbf{single-client, serial IVF search} only. FAISS (Facebook AI
Similarity Search) is an in-process library with minimal overhead; Milvus is a vector
\textbf{database} with gRPC, persistence (etcd/MinIO), collection lifecycle, and client
marshalling. For that narrow micro-benchmark, FAISS \emph{should} win---and it does.

Choose Milvus when the product needs what a library does not ship as one package:
\begin{itemize}[leftmargin=*]
  \item \textbf{Serving:} concurrent clients, REST/gRPC APIs, collections, metadata filtering.
  \item \textbf{Durability and ops:} data survives restarts, backups, replication, monitoring.
  \item \textbf{Scale-out:} sharded indexes across nodes (FAISS is single-process unless you
        build that layer yourself).
  \item \textbf{Integration target for this project:} AMD GPU search via hipVS/Knowhere in
        Milvus---not FAISS-in-a-script.
  \item \textbf{Production features:} RBAC, hybrid search, incremental ingest, etc.
\end{itemize}
The harness goal is a \textbf{reproducible CPU baseline before GPU work}, not to pick the
fastest raw search library. VectorDBBench adds optimize/compact overhead on first load; p99
latencies in Table~6 are sub-millisecond to a few ms (serial protocol).

\subsection{Does Milvus show an advantage here?}

\textbf{Not on raw search QPS in this harness.} Section above explains when Milvus still
makes sense despite FAISS being faster. The gap is expected given what each run measures:
\begin{itemize}[leftmargin=*]
  \item \textbf{FAISS:} in-process library with minimal overhead.
...
See ``Why use Milvus if FAISS is faster?'' above. These CPU tables additionally omit index
types beyond IVF\_FLAT (HNSW, IVF\_PQ), concurrent load, and multi-node sharding benchmarks.
