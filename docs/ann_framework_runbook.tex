...
The four measured-result tables answer different questions; we should not read them as a
...
\begin{itemize}[leftmargin=*]
  \item Table~1 (FAISS) and Tables~3--4 (Milvus on WSL2) run on \textbf{different hardware};
        we do not compare raw QPS numbers without a same-host re-run.
...
\begin{itemize}[leftmargin=*]
  ...
  \item We require a fairer engine comparison on the \textbf{same machine} (ideally the AMD
        host), with identical \texttt{nlist/nprobe} and, where possible, reduced Python
...
needed). We install it with Milvus client support:
\begin{verbatim}
...
We ensure Milvus Standalone is healthy before starting:
\begin{verbatim}
