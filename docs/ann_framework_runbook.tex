...
\begin{itemize}[leftmargin=*]
  ...
  \item Optionally run VectorDBBench against the same standalone URI (Section~\ref{sec:vectordbbench})
        for Zilliz-style benchmark reports.
\end{itemize}
...
\begin{itemize}[leftmargin=*]
  \item Keep \textbf{ann-harness-amd} for same-host FAISS vs Milvus IVF tuning and debugging
        (e.g.\ Milvus Lite \texttt{nprobe} issues).
  \item Add \textbf{VectorDBBench} when you need official Milvus numbers, concurrent search,
...
Prefer a \textbf{CLI dry-run first}, then a \textbf{serial-only} run inside \texttt{tmux}
(Section~\ref{sec:long-runs-ssh}). Skip the default concurrent search sweep on hosts with
unstable SSH.

VectorDBBench requires \textbf{Python $\geq$ 3.11} (separate venv from this harness if
needed). Install with Milvus client support:
\begin{verbatim}
python3.11 -m venv ~/vdbbench-venv
source ~/vdbbench-venv/bin/activate
pip install -U pip vectordb-bench
vectordbbench --help
vectordbbench milvusautoinindex --help
\end{verbatim}

Milvus Standalone must already be healthy:
\begin{verbatim}
cd milvus-docker && docker-compose ps
curl -sf http://127.0.0.1:9091/healthz && echo OK
\end{verbatim}

\subsection{Minimal command line (local Milvus Standalone)}

VectorDBBench exposes \texttt{milvusautoinindex} and \texttt{milvusfts} for Milvus;
...
\textbf{Step 1: dry-run} (validates URI and case without loading data):
\begin{verbatim}
vectordbbench milvusautoinindex \
...
Use \texttt{127.0.0.1} rather than \texttt{localhost} on WSL2 (IPv6 client issues).

\textbf{Step 2: real run} (after dry-run output looks correct):
\begin{verbatim}
tmux new -s vdbbench
LOG=logs/vdbbench_$(date -u +%Y%m%d_%H%M%S).log
mkdir -p logs
PYTHONUNBUFFERED=1 vectordbbench milvusautoinindex \
  --uri "http://127.0.0.1:19530" \
  --case-type CapacityDim128 \
  --k 10 \
  --drop-old \
...
VectorDBBench usually downloads datasets in its own layout; confirm
...
\begin{itemize}[leftmargin=*]
  ...
  \item \textbf{\texttt{init\_bench} Web UI alone} --- fine for exploration, harder to
        archive in the runbook; prefer CLI + \texttt{tee} for traceability.
