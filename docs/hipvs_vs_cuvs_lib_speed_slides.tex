...
\begin{frame}{SIFT product path --- recall OK, speed inconclusive}
  ...
  \vspace{0.5em}
  \begin{block}{Next fair Milvus speed compare}
    \footnotesize
    \textbf{GIST-1M} (960-d) + \texttt{GPU\_IVF\_PQ} $m{=}32$,
    \texttt{nprobe}$\in\{8,16,32,64,128\}$ --- same client both GPUs.\\
    Goal: AMD/NVIDIA $\le\sim$0.85$\times$ at matched recall
    (runbook: \texttt{docs/milvus\_amd\_behind\_nvidia\_plan.md}).
    Results slot: \texttt{results/milvus\_gist\_pq/}.
  \end{block}
\end{frame}
...
\begin{frame}{Takeaway for management}
  \begin{enumerate}
    ...
    \item \textbf{Milvus SIFT:} stack-limited --- $\sim$1$\times$ is \textbf{not}
          proof of ANN parity. Fair product speed trial = \textbf{GIST PQ} (pending lab).
  \end{enumerate}

  \vspace{0.6em}
  \begin{center}
    \large
    Correct port. Library gap is real on PQ.\\[0.15em]
    Product-path speed needs a GPU-heavy Milvus recipe.
  \end{center}

  \vspace{0.5em}
  {\footnotesize
  Lab dates 2026-07-23 (lib PQ) / 2026-07-27 (lib FLAT heavy).
  GIST Milvus: \texttt{docs/milvus\_amd\_behind\_nvidia\_plan.md}.}
\end{frame}
