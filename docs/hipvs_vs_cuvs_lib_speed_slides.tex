...
\begin{frame}{The question}
  ...
  \vspace{0.5em}
  {\footnotesize Not a Milvus product-path compare --- that is $\sim$1$\times$ (see last slides).}
\end{frame}
...
\begin{frame}{QPS --- hipVS vs cuVS (IVF\_PQ $m{=}32$)}
  \vspace{-0.2em}
  {\footnotesize Batched search. Higher is faster.}\\[0.1em]
  \centering
  \begin{tikzpicture}
    \begin{axis}[
      width=0.95\textwidth,
...
\begin{frame}{Speed-up ratio (hipVS / cuVS)}
  ...
  \begin{tikzpicture}
    \begin{axis}[
      ...
      \addplot+[thick,mark=*,teal] coordinates {
      ...
      \addplot[dashed,teal!60!black] coordinates {(0,0.5) (34,0.5)};
      \node[anchor=west,teal!60!black,font=\small] at (axis cs:20,0.55) {$\sim$0.5$\times$};
    \end{axis}
  \end{tikzpicture}
\end{frame}
...
\begin{frame}{Recall@10 --- same on both}
  \vspace{-0.2em}
  {\footnotesize Quality tracks; the gap is throughput, not accuracy.}\\[0.1em]
  \centering
  \begin{tikzpicture}
    \begin{axis}[
      width=0.95\textwidth,
...
%----- library vs milvus -----
\begin{frame}{Library vs Milvus (same GPUs, IVF\_PQ $m{=}32$)}
  \footnotesize
  Same peer GPUs and PQ recipe. \textbf{Library} $=$ direct \texttt{cuvs} API;
  ...
  \begin{tabular}{@{}llrrrr@{}}
    ...
    \midrule
    Milvus  & 8  & 19500 & 19786 & $0.99\times$ & $\sim$36$\times$ below lib \\
    Milvus  & 32 & 14784 & 14144 & $1.05\times$ & $\sim$15$\times$ below lib \\
    \bottomrule
  \end{tabular}
  \end{center}
  ...
  \vspace{0.35em}
  {\scriptsize
  ``Scale'' $=$ how much lower Milvus QPS is than library QPS on the same GPU
  (order-of-magnitude; AMD column).}\\[0.25em]
...
%----- product contrast -----
\begin{frame}{Contrast: under Milvus the gap disappears}
  \footnotesize
  Same GPUs, batched SIFT-1M \textbf{product path} (Knowhere + Milvus).
  Speed-up $=$ AMD QPS / CUDA QPS.\\[0.35em]
  ...
  \vspace{0.5em}
  \begin{block}{Why both can be true}
    \footnotesize
    Library microbench: ratio is kernel-dependent (PQ $\sim$0.5$\times$; FLAT heavy $\sim$1.3$\times$).\\
    Product path: RPC / Knowhere / batching dominate $\Rightarrow$ AMD $\sim$1$\times$ on SIFT FLAT/PQ.
  \end{block}
\end{frame}
...
\begin{frame}{Takeaway for management}
  \begin{enumerate}
    ...
    \item \textbf{In Milvus:} AMD \textbf{competitive} ($\sim$1$\times$); hipVS is not the
          product-path bottleneck on SIFT FLAT/PQ today.
  \end{enumerate}

  \vspace{0.6em}
  \begin{center}
    \large
    Correct port. Library gap depends on workload;\\[0.15em]
    product path already peer-competitive for FLAT/PQ.
  \end{center}

  \vspace{0.5em}
  {\footnotesize
  Lab dates 2026-07-23 (PQ) / 2026-07-27 (FLAT heavy).
  Details: \texttt{docs/hipvs\_vs\_cuvs\_bench.md},
