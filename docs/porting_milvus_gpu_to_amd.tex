...
\end{itemize}
Library microbenches (no Milvus) show a larger PQ gap ($\sim$0.5$\times$) and a
FLAT-heavy AMD lead ($\sim$1.3$\times$) --- useful context, not a product-QPS claim.
\end{abstract}
...
\textit{QPS vs \texttt{nprobe} --- pending both-GPU fill}\\[0.6em]
{\small Run \texttt{scripts/run\_milvus\_layer4\_gist\_flat.sh}
on AMD and CUDA (\texttt{nprobe}$\in\{8,16,32,64,128\}$),
then replace this box with the same plot style as Figure~\ref{fig:gist-pq}.}\\[0.4em]
{\small Expect higher recall and lower QPS than GIST PQ; VRAM $\sim$3.8\,GB raw floats.}
\vspace{1.2em}}}
\caption{GIST-1M, sealed \texttt{GPU\_IVF\_FLAT}, $nlist{=}1024$, $k{=}10$.
\textbf{Pending lab numbers} (script ready in the harness).}
\label{fig:gist-flat}
\end{figure}
...
\begin{itemize}[leftmargin=*]
  ...
  \item \textbf{Library context} (optional microbench, no Milvus):
        hipVS \texttt{IVF\_PQ} $m{=}32$ $\sim$0.37--0.55$\times$ cuVS;
        GPU-heavy \texttt{IVF\_FLAT} $\sim$1.3$\times$ hipVS/cuVS on the same peer GPUs
...
hipVS / hipRAFT & ROCm-DS upstream + harness gfx1100 patches & e.g.\ \texttt{release/rocmds-25.10} \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{itemize}[leftmargin=*]
  ...
  \item Optional PQ launch tuning (prefer \texttt{blockDim}${=}512$ on AMD) is tracked
        in harness \texttt{results/lib\_bench/LAUNCH\_KNOBS.md}; rebuild hipVS for
        dashboard/product binaries to pick it up.
\end{itemize}
