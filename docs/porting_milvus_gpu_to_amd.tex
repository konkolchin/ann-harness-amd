...
\begin{itemize}[leftmargin=*]
  ...
  \item \textbf{Library context} (optional microbench, no Milvus;
        Figure~\ref{fig:lib-pq}):
        with AMD-default \texttt{blockDim}${=}512$, hipVS \texttt{IVF\_PQ} $m{=}32$
        is $\sim$0.60--0.62$\times$ cuVS mid-grid (was $\sim$0.48--0.50$\times$);
        GPU-heavy \texttt{IVF\_FLAT} $\sim$1.3$\times$
...
NVIDIA-oriented occupancy heuristic. On \texttt{gfx1100}, forcing
\texttt{blockDim}${=}512$ raised library IVF-PQ search QPS by $\sim$25--35\%
at matched recall (harness A/B: \texttt{results/lib\_bench/LAUNCH\_KNOBS.md}).
...
\addlegendentry{hipVS 7900 (bt512 default)}
\addplot[blue!70!black, mark=triangle*, mark size=2.5pt, line width=1.0pt, dashed] coordinates {
  (1,1593707) (4,1268942) (8,710989) (16,393742) (32,217275)
};
\addlegendentry{hipVS 7900 (Jul stock)}
\addplot[green!70!black, mark=square*, mark size=2.5pt, line width=1.15pt] coordinates {
...
\end{axis}
\end{tikzpicture}
\caption{Library \texttt{IVF\_PQ} ($m{=}32$, SIFT-1M, no Milvus).
Recall@10 matched across series ($\approx0.35\rightarrow0.73$).
Default \texttt{blockDim}${=}512$ lifts mid-grid hipVS/cuVS from $\sim$0.5$\times$
to $\sim$0.60--0.62$\times$ (\texttt{FILLED\_pq\_m32.md}, 2026-08-15 confirm).}
\label{fig:lib-pq}
\end{figure}
