...
def wait_ready_for_search(client: MilvusClient, collection: str, timeout_s: float) -> None:
...
if do_flush:
    # Seal any rows inserted after the first flush; then wait for IndexNode.
    t0 = time.time()
    client.flush(args.collection)
    print(f"post_index_flush_time_s={time.time() - t0:.2f}")
    print(f"waiting up to {args.index_wait_s:.0f}s for sealed index / load...")
    wait_ready_for_search(client, args.collection, args.index_wait_s)
else:
