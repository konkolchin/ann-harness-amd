...
if [ "${_build_rc}" -ne 0 ]; then
  ...
  grep -nE 'CMake Error|Configuring incomplete|FATAL_ERROR|WITH_HIP=ON but WITH_CUVS|No rule to make target|undefined reference|ld\.lld:|linker command failed|ConanException|Error downloading|FAILED:' \
    "${LOG}" | grep -viE 'deprecated|FMT_DEPRECATED|-Wdeprecated|note:|warning:|CPU_ARCH|machdep\.cpu|get_cpu_arch' | tail -80 >&2 || true
  echo "" >&2
  echo "---- first CMake Error block ----" >&2
  grep -n -A12 'CMake Error' "${LOG}" | head -40 >&2 || true
  echo "" >&2
  echo "---- KNOWHERE / HIP flags (last match) ----" >&2
  grep -E 'WITH_CUVS=|WITH_HIP=|MILVUS Layer3|CMAKE_EXTRA_ARGS=|already added|CONFIGURED' "${LOG}" | tail -40 >&2 || true
  echo "" >&2
